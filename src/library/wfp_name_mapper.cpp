...
namespace
{
#define WFP_NAME(guid, friendlyName)                                                               \
    {                                                                                              \
        guid,                                                                                      \
        {                                                                                          \
            friendlyName, #guid                                                                    \
...
const std::unordered_map<GUID, WfpName> kGuidWfpNameMap{
    // Layer names, see full list here:
    ...
    // Condition types, see full list here:
...
// Match types, see full list here:
...
// IP Version types, see full list here:
// https://learn.microsoft.com/en-us/windows/win32/api/fwptypes/ne-fwptypes-fwp_ip_version
...
const std::unordered_map<UINT32, WfpName> kActionTypeMap{
    WFP_NAME(FWP_ACTION_BLOCK, "block"), WFP_NAME(FWP_ACTION_PERMIT, "permit"),
...
// See IPPROTO_* section of
// https://learn.microsoft.com/en-us/windows/win32/api/winsock2/nf-winsock2-socket
const std::unordered_map<UINT32, WfpName> kIpProtoTypeMap{
    WFP_NAME(IPPROTO_ICMP, "icmp"),        WFP_NAME(IPPROTO_IGMP, "igmp"),
    WFP_NAME(IPPROTO_TCP, "tcp"),          WFP_NAME(IPPROTO_UDP, "udp"),
    WFP_NAME(IPPROTO_ICMPV6, "ipv6 icmp"),
};

// Event types, see full list:
...
template <> WfpName WfpNameMapper::getName<WFPK_ACTION_TYPE, UINT32>(UINT32 value)
{
    return nameLookup(kActionTypeMap, value, "UNKNOWN-ACTIONTYPE");
}

template <> WfpName WfpNameMapper::getName<WFPK_IPPROTO_TYPE, UINT8>(UINT8 value)
{
