...
// PIA-specific data
inline constinit GUID PIA_PROVIDER_KEY = {
    0x8de3850, 0xa416, 0x4c47, {0xb3, 0xad, 0x65, 0x7c, 0x5e, 0xf1, 0x40, 0xfb}};
inline constinit GUID PIA_SUBLAYER_KEY = {
    0xf31e288d, 0xde5a, 0x4522, {0x94, 0x58, 0xde, 0x14, 0xeb, 0xd0, 0xa3, 0xf8}};

// Base error
class WfpError : public std::runtime_error
{
public:
    WfpError(const std::string &message, DWORD code,
             const std::source_location &loc = std::source_location::current())
        : std::runtime_error(formatErrorMessage(message, code, loc))
    {}

private:
    static std::string formatErrorMessage(const std::string &message, DWORD code,
                                          const std::source_location &loc)
    {
        return std::format("{} {} @ (file: {}, line: {})", message, getErrorString(code),
...
struct WfpDeleter
{
    template <typename WfpPtr> void operator()(WfpPtr *ptr) const
    {
        if(ptr)
        {
...
struct FilterCompare
{
    ...
    template <typename PtrT> bool operator()(const PtrT &lhs, const PtrT &rhs) const
    {
        ...
        switch(lhs->weight.type)
        {
            ...
            case FWP_UINT8:
            default: return lhs->weight.uint8 > rhs->weight.uint8;
        }
    }
};
...
public:
    ...
    void forEach(IterFuncT func) const
    {
        ...
        {
            SingleLayerFilterEnum{layer, _engineHandle}.forEach(
                [&](const auto &pFilter) { func(pFilter); });
        }
    }
...
public:
    ...
    void start(FuncT callbackFunc)
    {
        ...
        eventEnumTemplate.endTime.dwLowDateTime =
            eventEnumTemplate.startTime.dwHighDateTime + 10000;
        eventEnumTemplate.endTime.dwHighDateTime =
            eventEnumTemplate.startTime.dwHighDateTime + 1000;
        ...
        DWORD result = FwpmNetEventSubscribe(_engineHandle, &subscription, callbackFunc,
...
public:
    ...
    auto filtersForLayer(const GUID &layerKey) const -> FilterSet
    {
