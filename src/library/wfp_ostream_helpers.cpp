...
std::ostream &operator<<(std::ostream &os, const FWPM_NET_EVENT &event)
{
    ...
    if(!(header.ipProtocol == IPPROTO_ICMP || header.ipProtocol == IPPROTO_TCP ||
         header.ipProtocol == IPPROTO_UDP))
    {
        os << std::format(
            "Only some protocols are supported! Got unsupported procotol number: {}\n",
            header.ipProtocol);
        return os;
    }
    ...
    switch(event.type)
    {
        case FWPM_NET_EVENT_TYPE_CLASSIFY_DROP: filterId = event.classifyDrop->filterId; break;
        case FWPM_NET_EVENT_TYPE_CLASSIFY_ALLOW: filterId = event.classifyAllow->filterId; break;
    }
    ...
    std::string protocol =
        WfpNameMapper::getName<WFPK_IPPROTO_TYPE>(header.ipProtocol).friendlyName;
    ...
    os << std::format("[protocol: {}] [FilterId: {}] {} {} {}:{} -> {}:{}", protocol, filterId,
                      eventType, fileName, localAddress, header.localPort, remoteAddress,
                      header.remotePort);
...
std::ostream &operator<<(std::ostream &os, const FWPM_FILTER &filter)
{
    switch(filter.weight.type)
    {
        case FWP_EMPTY: // Weight managed by bfe
            ...
            break;
        case FWP_UINT8:
            os << std::format("[Id: {}] [Weight(8u): {:2}]", filter.filterId,
                              static_cast<int>(filter.weight.uint8));
            break;
        case FWP_UINT64:
            os << std::format("[Id: {}] [Weight(64u): {:2}]", filter.filterId,
                              *filter.weight.uint64);
            break;
    }
    ...
    else
    {
        ...
        for(size_t conditionCount = 0; conditionCount < filter.numFilterConditions;
            ++conditionCount)
        {
...
std::ostream &operator<<(std::ostream &os, const FWPM_FILTER_CONDITION &condition)
{
    ...
    {
        case FWP_EMPTY: os << "empty"; break;
        case FWP_UINT8: os << static_cast<UINT32>(condition.conditionValue.uint8); break;
        case FWP_INT8: os << static_cast<INT32>(condition.conditionValue.int8); break;
        case FWP_UINT16: os << static_cast<UINT32>(condition.conditionValue.uint16); break;
        case FWP_INT16: os << static_cast<INT32>(condition.conditionValue.int16); break;
        case FWP_UINT32: os << static_cast<UINT32>(condition.conditionValue.uint32); break;
        case FWP_INT32: os << static_cast<INT32>(condition.conditionValue.int32); break;
        case FWP_UINT64: os << *condition.conditionValue.uint64; break;
        case FWP_INT64: os << *condition.conditionValue.int64; break;
        ...
        default:
            os << std::format("unknown (type: {})",
                              static_cast<INT32>(condition.conditionValue.type));
    }
